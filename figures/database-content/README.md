# database-content — OmniPath Metabo database content
...
| Panel | Source | Notes |
|------:|:-------|:------|
| A | Vendored manual asset (`figures/architecture/manual/omnipath-architecture-new2026.pdf`) | FR-005 / FR-005a — SHA-256 fingerprint pinned in the asset folder's `README.md` and verified by `build.R` before include; mismatch fails the rebuild. |
...
- `out/database-content.{pdf,svg}` — bare composite (Panel A vendored
...
- `out/database-content-with-caption.pdf` — composite + typeset
  caption (xelatex + `tex/caption.sty`, FR-041).
- `out/caption.txt` — plain-text caption (deterministic strip,
  FR-041a).
- `out/database-content.pdf.provenance.json` — sidecar.
