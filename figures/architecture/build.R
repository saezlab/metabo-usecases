...
# to figures/database-content/. This script:
#
# - verifies the vendored architecture asset's SHA-256 against
#   figures/architecture/manual/README.md (FR-005a),
# - copies the vendored PDF + SVG to out/architecture.{pdf,svg},
...
setup_pipeline_log("build:architecture")
set.seed(pipeline_seed())

out_dir <- "figures/architecture/out"
...
architecture_dir    <- "figures/architecture/manual"
...
file.copy(
    architecture_pdf,
    file.path(out_dir, "architecture.pdf"),
    overwrite = TRUE
)
if (file.exists(architecture_svg)) {
    file.copy(
        architecture_svg,
        file.path(out_dir, "architecture.svg"),
        overwrite = TRUE
    )
}
...
digest_result <- build_panel_a_digest(
    ...
    out_dir     = "figures/architecture/panel-a-stats",
...
caption_info <- compose_caption(
    ...
    composite_pdf  = file.path(out_dir, "architecture.pdf"),
    caption_source = "figures/architecture/caption.tex",
    out_dir        = out_dir,
    panel_count    = 1L
)
...
write_sidecar(
    ...
    artifact_path  = file.path(out_dir, "architecture.pdf"),
    deployments    = list(dep5$deployment),
    manifests      = list(dep5$manifest),
    script_path    = "figures/architecture/build.R",
    queries        = list(),
...
pg_close_all_panel()
logger::log_info("architecture complete")
