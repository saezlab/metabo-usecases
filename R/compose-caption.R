...
caption_wrapper_tex <- function(composite_basename,
    ...
    # Asymmetric border to compensate for standalone's known
    # auto-bbox asymmetry: with multi-paragraph minipage content
    # (caption + image), the natural bbox under-counts the right
    # and bottom edges by ~15-20 mm. Empirically that places the
    # full border budget on top + left and leaves the right +
    # bottom flush against the page edge. Pre-loading the right
    # and bottom border values keeps a visible margin on every
    # side even after the bbox shift.
    c(
        "% Auto-generated by R/compose-caption.R — do not edit.",
        "\\documentclass[border={2mm 2mm 22mm 28mm}]{standalone}",
        "\\usepackage{graphicx}",
        "\\usepackage{caption}",
        "\\topskip=0pt",
        "\\parskip=0pt",
