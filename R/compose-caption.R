...
caption_wrapper_tex <- function(composite_basename,
    ...
    # No trailing \par — the \par causes TikZ to under-count the
    # bottom of the minipage's natural bbox (the line-skip glue
    # after \par isn't part of the typeset height). Without \par
    # the include is treated as inline content; the surrounding
    # \vspace lines provide the vertical separation.
    composite_block <- if (trim_mm > 0L) {
        sprintf(
            "  \\includegraphics[width=%s,trim=%s,clip]{%s}",
    ...
    # TikZ-based layout: wrap the minipage in a TikZ node so
    # standalone's bbox detection is exact (top/bottom and
    # left/right margins are symmetric). The plain
    # standalone+minipage path suffered from a known asymmetry
    # where the auto-detected bbox under-counted the right +
    # bottom edges; pinning the content inside a TikZ node with
    # anchor=north west and inner sep=0pt fixes the bbox to the
    # minipage's exact dimensions.
    #
    # Don't \usepackage{caption}: the metabo caption.sty redefines
    # \normalsize and \setsansfont in ways that propagate
    # additional baseline glue into the minipage's natural bbox,
    # which TikZ then mis-measures. Instead inline the only macro
    # the caption.tex sources actually depend on: \figpanel.
    c(
        ...
        "\\usepackage{fontspec}",
        "\\usepackage{xparse}",
        "\\IfFontExistsTF{Helvetica Neue LT Std}{%",
        "  \\setmainfont{Helvetica Neue LT Std}%",
        "}{}",
        "\\renewcommand{\\normalsize}{\\fontsize{8pt}{10pt}\\selectfont}",
        "\\normalsize",
        "\\NewDocumentCommand{\\figpanel}{m m}{(#1)~#2\\space}",
        "\\begin{document}",
        "\\begin{tikzpicture}",
