...
caption_wrapper_tex <- function(composite_basename,
    ...
    c(
        ...
        # border=8mm: standalone's auto-bbox detection has been
        # under-counting the right + bottom edges of the minipage
        # body — the rightmost column and the last data row were
        # rendering against the page edge with no margin. 8 mm
        # gives a comfortable margin while keeping the document
        # compact.
        "\\documentclass[border=8mm]{standalone}",
        "\\usepackage{graphicx}",
        "\\usepackage{caption}",
        "\\begin{document}",
        # \noindent so the minipage starts at the left margin rather
        # than the LaTeX default \parindent offset.
        "\\noindent",
