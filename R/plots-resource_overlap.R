...
#'
#' @param data Tibble from \code{\link{resource_overlap}}.
...
#'
#' @param node_sizes Tibble from \code{\link{resource_overlap_node_sizes}} with
...
#' @export
plot_resource_overlap <- function(data,
                                 min_overlap = 100L,
                                 width_mm    = 320L,
                                 node_sizes  = NULL) {
    ...
    if (is.null(node_sizes)) {
        node_sizes <- resource_overlap_node_sizes("database-content")
    }
