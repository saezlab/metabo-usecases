...
#'
#' @param data Tibble from \code{\link{resource_overview_overview}}.
...
#' @export
plot_resource_overview <- function(data,
                                 facet_order = c(
    ...
    for (i in seq_along(bands)) {
        ...
        band_data <- data[as.character(data$magnitude_band) == band, ,
                          drop = FALSE]
        plots[[i]] <- resource_overview_band_plot(
            band_data           = band_data,
            band_label          = band,
    ...
    su_legend <- resource_overview_extract_legend(
    ...
    facet_legends <- lapply(facet_order, function(f) {
        ...
        grob <- resource_overview_extract_legend(
            name   = paste0(f, " types"),
            values = cols,
...
#' @keywords internal
#' @noRd
resource_overview_extract_legend <- function(name, values, ncol = 2L) {
...
#' @param fill_map List from \code{\link{resource_overview_fill_map}}.
...
#' @keywords internal
#' @noRd
resource_overview_band_plot <- function(band_data,
                             band_label,
                             facet_order,
                             facet_class_colours,
                             ranking_facet,
...
#' @keywords internal
#' @noRd
resource_overview_fill_map <- function(data) {
    ...
    # per-facet-legend layout used by plot_resource_overview_total).
...
#' of \code{\link{plot_resource_overview_total}}:
#'   \itemize{
...
#' @keywords internal
#' @noRd
resource_overview_pretty_label <- function(x) {
...
#' @keywords internal
#' @noRd
resource_overview_su_legend_plot <- function() {
    df <- data.frame(
        x = c(1, 2),
        y = 1,
...
#'
#' Compact alternative to \code{\link{plot_resource_overview}}: instead
...
#' \code{\link{plot_resource_overview}} stays as a supplementary
#' artifact (\code{resource-overview-supplementary.pdf}).
#'
#' @param data Tibble from \code{\link{resource_overview_overview}}.
#' @param facet_order Character vector: facet column order.
...
#' @importFrom rlang .data
#' @export
plot_resource_overview_total <- function(data,
                              facet_order = c(
                                  "Entities", "Associations",
                                  "Interactions", "Identifiers",
                                  "Structures", "Literature"
    ...
    fill_map <- resource_overview_fill_map(data)
    ...
    sub_plots <- lapply(facet_order, function(f) {
        ...
        pretty_levels <- resource_overview_pretty_label(levels_f)
        ...
        facet_title <- resource_overview_pretty_label(f)
    ...
    all_plots <- c(list(resource_overview_su_legend_plot()), sub_plots)
