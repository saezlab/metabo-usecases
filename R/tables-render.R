...
tables_latex_wrapper <- function(body, max_width_mm = 180) {
    c(
        ...
        # border=0mm: the bare-table PDF carries no outer padding so
        # the with-caption layout can include it via \includegraphics
        # without trim+clip (which previously over-trimmed the
        # \extracolsep{\fill}-overflow slice at the right edge,
        # showing as the "thin slice cut off" the user reported).
        # The with-caption wrapper adds its own standalone border
        # so the outer document still has padding.
        sprintf(
            "\\documentclass[border=0mm,varwidth=%dmm]{standalone}",
...
tables_compose_caption <- function(table_id,
    ...
    compose_caption(
        ...
        # The bare PDF wrapper now uses border=0mm so the table
        # content occupies the full page; no trim is needed.
        trim_mm            = 0L
    )
}
